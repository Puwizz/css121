...
int main() {
    ...
    if (!found_query) {
        printf("Target ID %s not found in file!\n", target_id);
        return 1;
    }
    ...
    printf("Enter k (number of neighbors): ");
    if (scanf("%d", &k) != 1 || k <= 0 || k > cnt) {
    ...
    for (int p = 0; p < 4; p++) {
        ...
        predicted_mbti[p] = (cnt_a >= cnt_b) ? ch_a : ch_b;
    }
    predicted_mbti[4] = '\0';

    printf("Predicted MBTI for %s: %s\n", target_id, predicted_mbti);
